...
def init_db():
    ...
    conn.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS Lapin (
            idLapin INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        CREATE TABLE IF NOT EXISTS Reproduction (
            idRepro INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    conn.execute("INSERT OR IGNORE INTO Utilisateur (login,password,role) VALUES (?,?,?)", ("admin",pwd,"admin"))
    conn.commit()
    conn.close()

def log_action(action, detail=""):
    conn = get_conn()
    conn.execute("INSERT INTO Journal (utilisateur,action,detail) VALUES (?,?,?)",
                 (st.session_state.get("username","?"), action, detail))
    conn.commit()
    conn.close()
...
def export_rapport_mensuel(mois_str: str) -> bytes:
    ...
    total = conn.execute("SELECT COUNT(*) FROM Lapin WHERE statut='vivant'").fetchone()[0]
    portees_mois = conn.execute("SELECT COUNT(*), COALESCE(SUM(nbVivant),0), COALESCE(SUM(nbMort),0) FROM Portee WHERE strftime('%Y-%m',dateNaissance)=?", (mois_str,)).fetchone()
    ventes_mois = conn.execute("SELECT COUNT(*), COALESCE(SUM(prix),0) FROM Vente WHERE strftime('%Y-%m',dateVente)=?", (mois_str,)).fetchone()
    traitements = conn.execute("SELECT COUNT(*) FROM Sante WHERE strftime('%Y-%m',dateTraitement)=?", (mois_str,)).fetchone()[0]
    ...
    # Ventes détail
    df_v = pd.read_sql("SELECT l.nom, v.dateVente, v.prix, v.client FROM Vente v JOIN Lapin l ON v.idLapin=l.idLapin WHERE strftime('%Y-%m',v.dateVente)=?", conn, params=[mois_str])
    if not df_v.empty:
...
def get_alertes():
    ...
    lapins = pd.read_sql("SELECT idLapin, nom FROM Lapin WHERE statut='vivant'", conn)
    for _, l in lapins.iterrows():
        last = conn.execute("SELECT MAX(datePesee) FROM Pesee WHERE idLapin=?", (int(l['idLapin']),)).fetchone()[0]
        if last:
...
def get_elevage_context():
    conn = get_conn()
    ctx = {}
    ctx['total_vivants'] = conn.execute("SELECT COUNT(*) FROM Lapin WHERE statut='vivant'").fetchone()[0]
    ctx['males'] = conn.execute("SELECT COUNT(*) FROM Lapin WHERE sexe='M' AND statut='vivant'").fetchone()[0]
    ctx['femelles'] = conn.execute("SELECT COUNT(*) FROM Lapin WHERE sexe='F' AND statut='vivant'").fetchone()[0]
    ctx['total_portees'] = conn.execute("SELECT COUNT(*) FROM Portee").fetchone()[0]
    ctx['revenus_total'] = conn.execute("SELECT COALESCE(SUM(prix),0) FROM Vente").fetchone()[0]
    ctx['races'] = [r[0] for r in conn.execute("SELECT DISTINCT race FROM Lapin WHERE race != '' AND race IS NOT NULL").fetchall()]

    # Lapins liste
    lapins = conn.execute("SELECT nom, sexe, race, statut, dateNaissance FROM Lapin WHERE statut='vivant' LIMIT 20").fetchall()
...
def login_page():
    ...
    with col:
        ...
        if ok:
            ...
            conn = get_conn()
            user = conn.execute("SELECT login,role FROM Utilisateur WHERE login=? AND password=?", (login,h)).fetchone()
            conn.close()
            if user:
...
def page_dashboard():
    ...
    total    = conn.execute("SELECT COUNT(*) FROM Lapin WHERE statut='vivant'").fetchone()[0]
    males    = conn.execute("SELECT COUNT(*) FROM Lapin WHERE sexe='M' AND statut='vivant'").fetchone()[0]
    femelles = conn.execute("SELECT COUNT(*) FROM Lapin WHERE sexe='F' AND statut='vivant'").fetchone()[0]
    portees  = conn.execute("SELECT COUNT(*) FROM Portee").fetchone()[0]
    ventes   = conn.execute("SELECT COALESCE(SUM(prix),0) FROM Vente").fetchone()[0]
    traite   = conn.execute("SELECT COUNT(*) FROM Sante").fetchone()[0]
    nb_vivants_total = conn.execute("SELECT COALESCE(SUM(nbVivant),0) FROM Portee").fetchone()[0]
    taux_mort = round((conn.execute("SELECT COALESCE(SUM(nbMort),0) FROM Portee").fetchone()[0] / max(nb_vivants_total+1,1))*100, 1) if nb_vivants_total else 0
    ...
    # KPIs avancés
    moy_portee = round(conn.execute("SELECT AVG(nbVivant) FROM Portee").fetchone()[0] or 0, 1)
    best_f = conn.execute("""SELECT l.nom, SUM(p.nbVivant) as t FROM Portee p
    ...
    dernier_revenu = conn.execute("SELECT COALESCE(SUM(prix),0) FROM Vente WHERE strftime('%Y-%m',dateVente)=strftime('%Y-%m','now')").fetchone()[0]
    ...
    with col1:
        df_r = pd.read_sql("SELECT race, COUNT(*) as n FROM Lapin WHERE statut='vivant' AND race!='' GROUP BY race", conn)
        if not df_r.empty:
    ...
    with col3:
        df_p = pd.read_sql("SELECT datePesee, AVG(poids) as p FROM Pesee GROUP BY datePesee ORDER BY datePesee", conn)
        if not df_p.empty:
    ...
    with col4:
        df_v = pd.read_sql("SELECT strftime('%Y-%m',dateVente) as m, SUM(prix) as t FROM Vente GROUP BY m ORDER BY m", conn)
        if not df_v.empty:
    ...
    df_port = pd.read_sql("SELECT strftime('%Y-%m',dateNaissance) as m, COUNT(*) as p, SUM(nbVivant) as v, SUM(nbMort) as mo FROM Portee GROUP BY m ORDER BY m", conn)
    ...
    with c_ex1:
        df_all = pd.read_sql("SELECT * FROM Lapin", conn)
        xls = export_excel({"Lapins":df_all}, "Tous les lapins")
        st.download_button("📊 Excel — Lapins", xls, "lapins_ouakoubo.xlsx",
                           "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    with c_ex2:
        mois_dispo = pd.read_sql("SELECT DISTINCT strftime('%Y-%m',dateVente) as m FROM Vente WHERE dateVente IS NOT NULL ORDER BY m DESC", conn)
...
def page_lapins():
    ...
    with tabs[0]:
        ...
        if fs!="Tous": q+=" AND sexe=?"; p.append(fs)
        if ft!="Tous": q+=" AND statut=?"; p.append(ft)
        if fr: q+=" AND (race LIKE ? OR nom LIKE ?)"; p.extend([f"%{fr}%",f"%{fr}%"])
    ...
    with tabs[1]:
        if not role_ok("employe"): pass
            ...
            with st.form("fadd"):
                ...
                all_l=pd.read_sql("SELECT idLapin,nom,sexe FROM Lapin",conn)
                c4,c5=st.columns(2)
                ...
                if st.form_submit_button("💾 Enregistrer",use_container_width=True):
                    if not nom: st.error("Le nom est obligatoire.")
                        ...
                        conn.execute("INSERT INTO Lapin (nom,sexe,race,couleur,dateNaissance,origine,idPere,idMere,remarques,photo) VALUES (?,?,?,?,?,?,?,?,?,?)",
    ...
    with tabs[2]:
        all_l2=pd.read_sql("SELECT idLapin,nom FROM Lapin",conn)
        if all_l2.empty: st.info("Aucun lapin enregistré.")
        else:
            choix=st.selectbox("Choisir un lapin",all_l2['nom'].tolist())
            row=pd.read_sql("SELECT * FROM Lapin WHERE nom=?",conn,params=[choix]).iloc[0]
            ...
            cg, cs = st.columns(2)
            with cg:
                df_pw=pd.read_sql("SELECT datePesee,poids FROM Pesee WHERE idLapin=? ORDER BY datePesee",conn,params=[id_l])
                if not df_pw.empty:
            ...
            with cs:
                df_sa=pd.read_sql("SELECT dateTraitement,typeTraitement,produit FROM Sante WHERE idLapin=? ORDER BY dateTraitement DESC",conn,params=[id_l])
                if not df_sa.empty:
                    st.markdown("**💊 Santé**")
            ...
            with ce3:
                if role_ok("employe"):
                    ...
                    if st.button("💾 Mettre à jour"):
                        conn.execute("UPDATE Lapin SET statut=? WHERE idLapin=?",(new_st,id_l))
                        conn.commit()
    ...
    with tabs[3]:
        ...
        all_l3=pd.read_sql("SELECT idLapin,nom FROM Lapin",conn)
        if all_l3.empty: st.info("Aucun lapin.")
        else:
            lapin_sel=st.selectbox("Lapin central",all_l3['nom'].tolist(),key="gen_sel")
            row_g=pd.read_sql("SELECT * FROM Lapin WHERE nom=?",conn,params=[lapin_sel]).iloc[0]
            pere_nom=conn.execute("SELECT nom FROM Lapin WHERE idLapin=?",(row_g['idPere'],)).fetchone() if row_g['idPere'] else None
            mere_nom=conn.execute("SELECT nom FROM Lapin WHERE idLapin=?",(row_g['idMere'],)).fetchone() if row_g['idMere'] else None
            enfants=pd.read_sql("SELECT nom,sexe FROM Lapin WHERE idPere=? OR idMere=?",conn,params=[int(row_g['idLapin']),int(row_g['idLapin'])])
...
def page_reproduction():
    ...
    conn=get_conn()
    males=pd.read_sql("SELECT idLapin,nom FROM Lapin WHERE sexe='M' AND statut='vivant'",conn)
    femelles=pd.read_sql("SELECT idLapin,nom FROM Lapin WHERE sexe='F' AND statut='vivant'",conn)
        ...
        with st.form("frep"):
            if males.empty or femelles.empty:
                ...
                if st.form_submit_button("💾 Enregistrer",use_container_width=True):
                    ...
                    conn.execute("INSERT INTO Reproduction (idMale,idFemelle,dateAccouplement,dateControle,dateMiseBas,notes) VALUES (?,?,?,?,?,?)",
    ...
    # Confirmer gestation
    df_rep=pd.read_sql("""SELECT r.idRepro, lm.nom||' × '||lf.nom AS Couple,
        ...
        ORDER BY r.dateAccouplement DESC""",conn)
        ...
        for _,r in df_rep.iterrows():
            ...
            if not r['gestationConfirmee']:
                if cc3.button("Confirmer", key=f"conf_{r['idRepro']}"):
                    conn.execute("UPDATE Reproduction SET gestationConfirmee=1 WHERE idRepro=?",(int(r['idRepro']),))
                    conn.commit()
                    st.rerun()
...
def page_portees():
    ...
    conn=get_conn()
    repros=pd.read_sql("SELECT r.idRepro, lm.nom||' × '||lf.nom AS Couple FROM Reproduction r JOIN Lapin lm ON r.idMale=lm.idLapin JOIN Lapin lf ON r.idFemelle=lf.idLapin",conn)
        ...
        with st.form("fpes"):
            if lapins.empty:
                ...
                if st.form_submit_button("💾 Enregistrer",use_container_width=True):
                    ...
                    conn.execute("INSERT INTO Pesee (idLapin,datePesee,poids) VALUES (?,?,?)",(il,str(dp),po))
                    conn.commit()
                    log_action("PESEE",f"{ls}: {po}g")
                    st.success(f"✅ {po}g pour {ls}")
    df=pd.read_sql("SELECT l.nom AS Lapin, p.datePesee AS Date, p.poids AS Poids_g FROM Pesee p JOIN Lapin l ON p.idLapin=l.idLapin ORDER BY p.datePesee",conn)
    if not df.empty:
    ...
    with st.expander("➕ Ajouter un traitement",expanded=True):
        with st.form("fsan"):
            if lapins.empty:
                ...
                if st.form_submit_button("💾 Enregistrer",use_container_width=True):
                    il=int(lapins[lapins.nom==ls]['idLapin'].values[0])
                    conn.execute("INSERT INTO Sante (idLapin,dateTraitement,typeTraitement,produit,remarque,dateRappel) VALUES (?,?,?,?,?,?)",
                                 (il,str(dt),tt,pr,rm,str(rappel) if rappel else None))
                    conn.commit()
                    log_action("SANTE",f"{ls}: {tt} — {pr}")
                    st.success("✅ Traitement enregistré !")
    df=pd.read_sql("SELECT l.nom AS Lapin, s.dateTraitement AS Date, s.typeTraitement AS Type, s.produit AS Produit, s.dateRappel AS Rappel, s.remarque AS Remarques FROM Sante s JOIN Lapin l ON s.idLapin=l.idLapin ORDER BY s.dateTraitement DESC",conn)
    ...
    with st.expander("➕ Enregistrer une vente",expanded=True):
        with st.form("fven"):
            if lapins.empty:
                ...
                if st.form_submit_button("💾 Enregistrer",use_container_width=True):
                    il=int(lapins[lapins.nom==ls]['idLapin'].values[0])
                    conn.execute("INSERT INTO Vente (idLapin,dateVente,prix,client) VALUES (?,?,?,?)",(il,str(dv),px_,cl))
                    conn.execute("UPDATE Lapin SET statut='vendu' WHERE idLapin=?",(il,))
                    conn.commit()
                    log_action("VENTE",f"{ls}: {px_:,.0f} FCFA à {cl}")
                    st.success(f"✅ {ls} vendu — {px_:,.0f} FCFA")
    df=pd.read_sql("SELECT l.nom AS Lapin, v.dateVente AS Date, v.prix AS Prix_FCFA, v.client AS Client FROM Vente v JOIN Lapin l ON v.idLapin=l.idLapin ORDER BY v.dateVente DESC",conn)
    if not df.empty:
        c1,c2=st.columns(2)
...
def page_stocks():
    ...
    with tabs[0]:
        df=pd.read_sql("SELECT * FROM Stock ORDER BY dateAchat DESC",conn)
        if not df.empty:
    ...
    with tabs[1]:
        with st.form("fstk"):
            ...
            if st.form_submit_button("💾 Enregistrer",use_container_width=True):
                conn.execute("INSERT INTO Stock (type,nom,quantite,unite,prixUnitaire,dateAchat,notes) VALUES (?,?,?,?,?,?,?)",
                             (tp,nm,qt,ut,pu,str(da),nt))
                conn.commit()
                log_action("STOCK",f"Ajout {nm}: {qt}{ut}")
                st.success(f"✅ {nm} ajouté au stock !")
    with tabs[2]:
        df2=pd.read_sql("SELECT type, SUM(quantite*prixUnitaire) as total FROM Stock GROUP BY type",conn)
        if not df2.empty:
...
def page_calendrier():
    ...
    evenements=[]
    # Contrôles gestation
    ctrl=pd.read_sql("SELECT r.dateControle, lm.nom AS male, lf.nom AS femelle FROM Reproduction r JOIN Lapin lm ON r.idMale=lm.idLapin JOIN Lapin lf ON r.idFemelle=lf.idLapin WHERE r.gestationConfirmee=0 AND r.dateControle IS NOT NULL",conn)
    for _,r in ctrl.iterrows():
        evenements.append({"Date":str(r['dateControle']),"Type":"🔬 Contrôle gestation","Détail":f"{r['male']} × {r['femelle']}","Priorité":"Moyen"})

    # Mises bas prévues
    mb=pd.read_sql("SELECT r.dateMiseBas, lm.nom AS male, lf.nom AS femelle FROM Reproduction r JOIN Lapin lm ON r.idMale=lm.idLapin JOIN Lapin lf ON r.idFemelle=lf.idLapin WHERE r.gestationConfirmee=1 AND r.dateMiseBas IS NOT NULL",conn)
    for _,r in mb.iterrows():
        evenements.append({"Date":str(r['dateMiseBas']),"Type":"🍼 Mise bas","Détail":f"{r['male']} × {r['femelle']}","Priorité":"Haute"})

    # Rappels vaccin
    rap=pd.read_sql("SELECT l.nom, s.typeTraitement, s.dateRappel FROM Sante s JOIN Lapin l ON s.idLapin=l.idLapin WHERE s.dateRappel IS NOT NULL",conn)
...
def page_utilisateurs():
    ...
    with tabs[0]:
        df=pd.read_sql("SELECT id, login, role, dateCreation FROM Utilisateur",conn)
        st.dataframe(df,use_container_width=True)
        ...
        with st.form("fusr"):
            ...
            if st.form_submit_button("➕ Créer l'utilisateur",use_container_width=True):
                if not new_login or not new_pwd:
                    ...
                    try:
                        conn.execute("INSERT INTO Utilisateur (login,password,role) VALUES (?,?,?)",(new_login,h,new_role))
                        conn.commit()
...
def page_journal():
    ...
    if not role_ok("admin"): return
    conn=get_conn()
    df=pd.read_sql("SELECT dateAction, utilisateur, action, detail FROM Journal ORDER BY dateAction DESC LIMIT 200",conn)
...
def page_parametres():
    ...
    with st.form("fpwd"):
        ...
        if st.form_submit_button("🔒 Mettre à jour",use_container_width=True):
            ...
            user=conn.execute("SELECT * FROM Utilisateur WHERE login=? AND password=?",(st.session_state['username'],ah)).fetchone()
            if not user: st.error("❌ Ancien mot de passe incorrect.")
                ...
                conn.execute("UPDATE Utilisateur SET password=? WHERE login=?",(nh,st.session_state['username']))
                conn.commit()
    ...
    if os.path.exists(DB):
        with open(DB,"rb") as f:
            st.download_button("📥 Télécharger la base de données",f.read(),f"backup_ouakoubo_{datetime.now().strftime('%Y%m%d_%H%M')}.db","application/octet-stream")
    st.caption("Conservez une copie régulière pour ne pas perdre vos données.")
